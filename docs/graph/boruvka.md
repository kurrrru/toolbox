...
toolbox::graph::boruvka_edges(
    int n,
    const std::vector<std::tuple<long long, long long, long long>> &edges
);
template <typename Vertex, typename Cost>
std::optional<std::vector<std::tuple<Cost, Vertex, Vertex>>> toolbox::graph::boruvka_edges(
    int n,
    const std::vector<std::tuple<Cost, Vertex, Vertex>> &edges
);
```
...
- `boruvka` の戻り値: 最小全域木の総コスト。グラフが連結でない場合は `std::nullopt`
- `boruvka_edges` の戻り値: 最小全域木を構成する辺の集合(各辺 `{コスト, u, v}`、ラウンドをまたいで併合した順)。連結な場合はちょうど $n-1$ 本、頂点1個なら空、非連結なら `std::nullopt`。`boruvka` は内部的に `boruvka_edges` の結果を合計している

...
bool test_access(std::pair<std::vector<uint32_t>, toolbox::datastructure::wavelet_tree> test_case) {
    ...
    for (std::size_t i = 0; i < arr.size(); ++i) {
        if (wt.access(i) != arr[i]) {
            std::cerr << "Access failed at index " << i << ": expected " << arr[i] << ", got "
                      << wt.access(i) << std::endl;
            return false;
        }
    }
    return true;
...
bool test_rank(std::pair<std::vector<uint32_t>, toolbox::datastructure::wavelet_tree> test_case) {
    ...
    for (std::size_t i = 0; i < arr.size(); ++i) {
        if (wt.rank(i, arr[i]) != std::count(arr.begin(), arr.begin() + i, arr[i])) {
            std::cerr << "Rank failed at index " << i << ": expected "
                      << std::count(arr.begin(), arr.begin() + i, arr[i]) << ", got "
                      << wt.rank(i, arr[i]) << std::endl;
            return false;
        }
    }
    return true;
...
bool test_range_count(
    std::pair<std::vector<uint32_t>, toolbox::datastructure::wavelet_tree> test_case) {
    const auto &arr = test_case.first;
    const auto &wt = test_case.second;
    uint32_t lower = 3, upper = 7;
    std::size_t i = 2, j = 8;
    uint32_t expected_count =
        std::count_if(arr.begin() + i, arr.begin() + j,
                      [lower, upper](uint32_t v) { return v >= lower && v < upper; });
    if (wt.range_count(i, j, lower, upper) != expected_count) {
        std::cerr << "Range count failed: expected " << expected_count << ", got "
                  << wt.range_count(i, j, lower, upper) << std::endl;
        return false;
    }
    return true;
}

bool test_range_list(
    std::pair<std::vector<uint32_t>, toolbox::datastructure::wavelet_tree> test_case) {
    const auto &arr = test_case.first;
    const auto &wt = test_case.second;
    uint32_t lower = 3, upper = 7;
...
int main() {
    {
        ...
        std::vector<uint32_t> arr = {1, 3, 5, 7, 2, 4, 6, 8, 3, 5, 7, 9, 2, 4, 6, 8,
                                     1, 3, 5, 7, 2, 4, 6, 8, 3, 5, 7, 9, 1, 3, 5, 7};
        toolbox::datastructure::wavelet_tree wt(arr);

        toolbox::test_utils::runTests<
            std::pair<std::vector<uint32_t>, toolbox::datastructure::wavelet_tree>>(

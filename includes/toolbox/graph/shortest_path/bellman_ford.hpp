 ...
 * @return Shortest distance from start vertex if there is no negative cycle, otherwise an empty
 * vector
...
std::vector<Cost> bellman_ford(const Vertex s,
...
std::vector<long long> bellman_ford(
    const long long s, const std::vector<std::vector<std::pair<long long, long long>>> &cost) {
 ...
 * @note [complexity] O(VE)
 */
bool negative_cycle(const long long s,

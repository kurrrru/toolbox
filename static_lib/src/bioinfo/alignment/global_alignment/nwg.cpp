	...
	int a = 0, int x = 1, int o = 0, int e = 1)
{
	...
	for (int i = 1; i <= n; i++)
		for (int j = 1; j <= m; j++)
		{
			I[i][j] = std::min(M[i - 1][j] + o + e, I[i - 1][j] + e);
			D[i][j] = std::min(M[i][j - 1] + o + e, D[i][j - 1] + e);
			M[i][j] = std::min(M[i - 1][j - 1] + (s1[i - 1] == s2[j - 1] ? a : x), std::min(I[i][j], D[i][j]));
		}
	return (M[n][m]);
}
	...
	int a = 0, int x = 1, int o = 0, int e = 1)
{
	...
	while (i > 0 || j > 0)
	{
		...
		else if (i > 0 && table_id == table::dp_I)
		{
			s1_aligned = s1[i - 1] + s1_aligned;
			s2_aligned = "-" + s2_aligned;
			i--;
			if (i > 0 && I[i][j] == M[i - 1][j] + o + e)
				table_id = table::dp_M;
		}
		else if (j > 0 && table_id == table::dp_D)
		{
			s1_aligned = "-" + s1_aligned;
			s2_aligned = s2[j - 1] + s2_aligned;
			j--;
			if (j > 0 && D[i][j] == M[i][j - 1] + o + e)
				table_id = table::dp_M;
		}
	}
}
	...
	std::string &s1_aligned, std::string &s2_aligned, int a = 1, int x = 1, int o = 0, int e = 1)
{
	...
	int diff = needleman_wunsch_gotoh_dp(s1, s2, M, I, D, a, x, o, e);
	needleman_wunsch_gotoh_traceback(s1, s2, M, I, D, s1_aligned, s2_aligned, a, x, o, e);
	return (diff);
}

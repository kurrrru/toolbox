...
fm_index::fm_index(const fm_index &) = default;
fm_index::fm_index(fm_index &&) = default;
fm_index &fm_index::operator=(const fm_index &) = default;
fm_index &fm_index::operator=(fm_index &&) = default;
fm_index::~fm_index() = default;

fm_index::fm_index(const std::string &s) {build(s);}
fm_index::fm_index(const std::string &s, const std::string &order) {build(s, order);}
...
*/
int fm_index::count(const std::string &p)
{
	int m = p.size();
	int l = 0, r = _n - 1;
	for (int i = m - 1; i >= 0; i--)
...
* @note [complexity]: O(m), where m is the length of the pattern.
*/
std::vector<int> fm_index::locate(const std::string &p)
{
	int m = p.size();
	int l = 0, r = _n - 1;
	for (int i = m - 1; i >= 0; i--)
...
void fm_index::build(const std::string &s)
{
	_n = s.size();
	_s = s;
...
void fm_index::build(const std::string &s, const std::string &order)
{
	_n = s.size();
	_s = s;

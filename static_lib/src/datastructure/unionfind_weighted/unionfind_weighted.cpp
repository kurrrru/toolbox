...
unionfind_weighted::unionfind_weighted(const unionfind_weighted &) = default;
unionfind_weighted::unionfind_weighted(unionfind_weighted &&) = default;
unionfind_weighted &unionfind_weighted::operator=(const unionfind_weighted &) = default;
unionfind_weighted &unionfind_weighted::operator=(unionfind_weighted &&) = default;
unionfind_weighted::~unionfind_weighted() = default;
...
* @note [complexity]: O(n)
*/
unionfind_weighted::unionfind_weighted(int n)
{
	assert(n > 0);
	_n = n;
	_p.resize(n, -1);
...
*/
int unionfind_weighted::find(int x)
{
	assert(0 <= x && x < _n);
	if (_p[x] == -1)
		return x;
...
* @note [complexity]: O(alpha(n)), where alpha(n) is the inverse Ackermann function.
*/
bool unionfind_weighted::unite(int x, int y, int w)
{
	assert(0 <= x && x < _n);
	assert(0 <= y && y < _n);
	w += weight(x);
...
* @note [complexity]: O(alpha(n)), where alpha(n) is the inverse Ackermann function.
*/
bool unionfind_weighted::same(int x, int y)
{
	assert(0 <= x && x < _n);
	assert(0 <= y && y < _n);
...
* @note [complexity]: O(alpha(n)), where alpha(n) is the inverse Ackermann function.
*/
int unionfind_weighted::weight(int x)
{
	assert(0 <= x && x < _n);
	find(x);
...
* @note [complexity]: O(alpha(n)), where alpha(n) is the inverse Ackermann function.
*/
int unionfind_weighted::diff(int x, int y)
{
	assert(0 <= x && x < _n);
	assert(0 <= y && y < _n);

...
int needleman_wunsch_dp(const std::string &s1, const std::string &s2,
    ...
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            dp[i][j] = std::min(dp[i - 1][j - 1] + (s1[i - 1] == s2[j - 1] ? a : x),
                                std::min(dp[i - 1][j] + g, dp[i][j - 1] + g));
        }
    }
    return (dp[m][n]);
}
 ...
 */
void needleman_wunsch_traceback(const std::string &s1, const std::string &s2,
                                const std::vector<std::vector<int>> &dp, std::string &s1_aligned,
                                std::string &s2_aligned, int a = 0, int x = 1, int g = 1) {
    const int m = static_cast<int>(s1.size());
    const int n = static_cast<int>(s2.size());
    int i = m;
    int j = n;
    ...
    while (i > 0 || j > 0) {
        if (i > 0 && (dp[i][j] == dp[i - 1][j] + g || j == 0)) {
            ...
            j--;
                   ...
                   dp[i][j] == dp[i - 1][j - 1] + (s1[i - 1] == s2[j - 1] ? a : x)) {
            s1_aligned = s1[i - 1] + s1_aligned;
            s2_aligned = s2[j - 1] + s2_aligned;
            i--;
            j--;
 ...
 * @note O(nm) time complexity and O(nm) space complexity.
 */
int needleman_wunsch_all(const std::string &s1, const std::string &s2, std::string &s1_aligned,

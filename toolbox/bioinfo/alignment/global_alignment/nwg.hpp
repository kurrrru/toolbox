...
int needleman_wunsch_gotoh_dp(const std::string &s1, const std::string &s2,
                              std::vector<std::vector<int>> &M, std::vector<std::vector<int>> &D,
                              std::vector<std::vector<int>> &I, int a = 0, int x = 1, int o = 0,
                              int e = 1) {
    const int m = static_cast<int>(s1.size());
    const int n = static_cast<int>(s2.size());
    M.assign(m + 1, std::vector<int>(n + 1, 0));
    D.assign(m + 1, std::vector<int>(n + 1, 0));
    I.assign(m + 1, std::vector<int>(n + 1, 0));
    ...
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            ...
            M[i][j] = std::min(M[i - 1][j - 1] + (s1[i - 1] == s2[j - 1] ? a : x),
                               std::min(D[i][j], I[i][j]));
        }
    }
    return (M[m][n]);
...
void needleman_wunsch_gotoh_traceback(const std::string &s1, const std::string &s2,
                                      ...
                                      std::string &s1_aligned, std::string &s2_aligned, int a = 0,
...
int needleman_wunsch_gotoh_all(const std::string &s1, const std::string &s2,
                               std::string &s1_aligned, std::string &s2_aligned, int a = 1,
                               int x = 1, int o = 0, int e = 1) {

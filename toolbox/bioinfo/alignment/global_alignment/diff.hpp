 ...
 */
int diff_dp(const std::string &s1, const std::string &s2, std::vector<std::vector<int>> &M,
            int x = 1, int g = 1) {
    ...
    for (d = 1; d <= N; d++) {
        ...
        for (k = -d; k < d + 1; k++) {
            ...
            if (M[d][d + k] >= 0) {
                while (M[d][d + k] - k < m && M[d][d + k] < n &&
                       s1[M[d][d + k] - k] == s2[M[d][d + k]]) {
                    M[d][d + k]++;
                }
            }
...
void diff_traceback(const std::string &s1, const std::string &s2, std::vector<std::vector<int>> &M,
    ...
    while (d > 0) {
        ...
        if (d - g >= 0 && k - 1 >= -(d - g) && k - 1 <= (d - g) &&
            M[d - g][(d - g) + (k - 1)] + 1 >= now_max) {
            now_max = M[d - g][(d - g) + (k - 1)] + 1;
            src = 1;
        }
        if (d - g >= 0 && k + 1 >= -(d - g) && k + 1 <= d - g &&
            M[d - g][(d - g) + (k + 1)] >= now_max) {
    ...
    for (d = path.size() - 1; d >= 0; d--) {
        i = path[d].first;
        j = path[d].second;
        i_prev = (d == static_cast<int>(path.size()) - 1) ? 0 : path[d + 1].first;
        j_prev = (d == static_cast<int>(path.size()) - 1) ? 0 : path[d + 1].second;
 ...
 * @note O(nd) time complexity and O(d^2) space complexity.
 */
int diff_all(const std::string &s1, const std::string &s2, std::string &s1_aligned,
             std::string &s2_aligned, int x = 1, int g = 1) {

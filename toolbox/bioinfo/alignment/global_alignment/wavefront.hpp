 ...
 */
int wavefront_dp(const std::string &s1, const std::string &s2, std::vector<std::vector<int>> &M,
                 std::vector<std::vector<int>> &I, std::vector<std::vector<int>> &D, int x = 1,
                 int o = 0, int e = 1) {
    const int m = static_cast<int>(s1.size());
    const int n = static_cast<int>(s2.size());
    const int N = std::max(m, n) * (x + o + e);
    int s;
    int k;
    M.clear();
    I.clear();
    ...
    for (s = 1; s <= N; s++) {
        ...
        for (k = -s; k < s + 1; k++) {
            if (s - o - e >= 0 && k - 1 >= -(s - o - e) && k - 1 <= s - o - e &&
                M[s - o - e][(s - o - e) + (k - 1)] >= 0) {
                I[s][s + k] = std::max(I[s][s + k], M[s - o - e][(s - o - e) + (k - 1)] + 1);
            }
            if (s - e >= 0 && k - 1 >= -(s - e) && k - 1 <= s - e &&
                I[s - e][(s - e) + (k - 1)] >= 0) {
            ...
            if (M[s][s + k] >= 0) {
                while (M[s][s + k] - k < m && M[s][s + k] < n &&
                       s1[M[s][s + k] - k] == s2[M[s][s + k]]) {
                    M[s][s + k]++;
                }
            }
            if (M[s][s + k] - k == m && M[s][s + k] == n) {
                return (M.size() - 1);
            }
        }
    }
...
void wavefront_traceback(const std::string &s1, const std::string &s2,
                         ...
                         const std::vector<std::vector<int>> &D, std::string &s1_aligned,
                         std::string &s2_aligned, int x = 1, int o = 0, int e = 1) {
    const int m = static_cast<int>(s1.size());
    const int n = static_cast<int>(s2.size());
    ...
    while (s > 0) {
        if (table_id == table::dp_M) {
            if (s - x >= 0 && k >= -(s - x) && k <= (s - x) &&
                M[s - x][(s - x) + k] + 1 > I[s][s + k] &&
                M[s - x][(s - x) + k] + 1 > D[s][s + k]) {
                s = s - x;
            ...
            if (s - o - e >= 0 && k - 1 >= -(s - o - e) && k - 1 <= s - o - e &&
                I[s][s + k] == M[s - o - e][(s - o - e) + (k - 1)] + 1) {
                s = s - o - e;
                k = k - 1;
                path.push_back(std::make_pair(M[s][s + k] - k, M[s][s + k]));
                table_id = table::dp_M;
            } else if (s - e >= 0 && k - 1 >= -(s - e) && k - 1 <= s - e &&
                       I[s][s + k] == I[s - e][(s - e) + (k - 1)] + 1) {
                s = s - e;
                k = k - 1;
                path.push_back(std::make_pair(I[s][s + k] - k, I[s][s + k]));
                table_id = table::dp_I;
            }
        } else if (table_id == table::dp_D) {
            if (s - o - e >= 0 && k + 1 >= -(s - o - e) && k + 1 <= s - o - e &&
                D[s][s + k] == M[s - o - e][(s - o - e) + (k + 1)]) {
                s = s - o - e;
                k = k + 1;
                path.push_back(std::make_pair(M[s][s + k] - k, M[s][s + k]));
                table_id = table::dp_M;
            } else if (s - e >= 0 && k + 1 >= -(s - e) && k + 1 <= s - e &&
                       D[s][s + k] == D[s - e][(s - e) + (k + 1)]) {
                s = s - e;
                k = k + 1;
                path.push_back(std::make_pair(D[s][s + k] - k, D[s][s + k]));
                table_id = table::dp_D;
            }
        }
    }
    ...
    for (s = path.size() - 1; s >= 0; s--) {
        i = path[s].first;
        j = path[s].second;
        i_prev = (s == static_cast<int>(path.size()) - 1) ? 0 : path[s + 1].first;
        j_prev = (s == static_cast<int>(path.size()) - 1) ? 0 : path[s + 1].second;
 ...
 * @note O(nd) time complexity and O(d^2) space complexity.
 */
int wavefront_all(const std::string &s1, const std::string &s2, std::string &s1_aligned,
                  std::string &s2_aligned, int x = 1, int o = 0, int e = 1) {
